...
static void stats_update(struct WU_PortfolioStats_* stats, double cash,
    ...
    wu_indicator_update(stats->max_drawdown, portfolio_value);
    wu_indicator_update(stats->sharpe_ratio, perf);
    wu_indicator_update(stats->sortino_ratio, perf);
    wu_indicator_update(stats->calmar_ratio, perf);
}
...
static void stats_record_trade(struct WU_PortfolioStats_* stats, double pnl, 
    ...
    if (stats->mean) wu_indicator_update(stats->mean, pnl);
    if (stats->stdev) wu_indicator_update(stats->stdev, pnl);
    if (pnl > 0) {
...
static char* stats_to_keyvalue(struct WU_PortfolioStats_* stats) {
    ...
    // double pnl_mean = stats->mean ? wu_indicator_get(stats->mean) : NAN;
    double pnl_stdev = stats->stdev ? wu_indicator_get(stats->stdev) : NAN;
    ...
    int offset = snprintf(result, 8192,
        ...
        "win_rate=%.2f max_win=%.2f max_loss=%.2f avg_pnl=%.2f pnl_stdev=%.2f "
...
static void stats_free(struct WU_PortfolioStats_* stats) {
    ...
    wu_indicator_delete(stats->max_drawdown);
    wu_indicator_delete(stats->sharpe_ratio);
    wu_indicator_delete(stats->sortino_ratio);
    wu_indicator_delete(stats->calmar_ratio);
    wu_indicator_delete(stats->mean);
    wu_indicator_delete(stats->stdev);
    free(stats);
}
...
WU_PortfolioStats wu_portfolio_stats_new(double initial_cash, double risk_free_rate) {
    ...
    stats->num_assets = 0;
    stats->capacity = 0;
    // TODO: clean-up in case of getting any NULL
    ...
    stats->mean = wu_mean_new();
    stats->stdev = wu_stdev_new(1);
    return stats;
}
