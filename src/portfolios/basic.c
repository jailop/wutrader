...
static double calculate_position_size(WU_BasicPortfolio portfolio, WU_Signal signal) {
    ...
    if (ps->size_type == WU_POSITION_SIZE_ABS) {
    ...
    else if (ps->size_type == WU_POSITION_SIZE_PCT) {
        double available_cash = portfolio->cash;
        double max_value = available_cash * ps->size_value;
        return max_value / price;
    }
    else if (ps->size_type == WU_POSITION_SIZE_PCT_EQUAL) {
        double portfolio_value = portfolio->cash;
        for (int i = 0; i < portfolio->num_assets; i++) {
            WU_PositionVector* vec = portfolio->positions[i];
            double last_price = vec->last_price;
            for (int j = 0; j < vec->capacity; j++) {
                bool found = false;
                struct WU_Position_ pos = wu_position_get(vec, j, &found);
                if (found) {
                    portfolio_value += pos.quantity * last_price;
                }
            }
        }
        double allocation_per_asset = portfolio_value / portfolio->num_assets;
        double target_value = allocation_per_asset * ps->size_value;
        return target_value / price;
    }
    else if (ps->size_type == WU_POSITION_SIZE_STRATEGY_GUIDED) {
        double portfolio_value = portfolio->cash;
        for (int i = 0; i < portfolio->num_assets; i++) {
            WU_PositionVector* vec = portfolio->positions[i];
            double last_price = vec->last_price;
        ...
        double target_value = portfolio_value * target_proportion;
        target_value *= ps->size_value;
        return target_value / price;
    }
    return 0.0;
}
...
static void execute_buy(WU_BasicPortfolio portfolio, WU_Signal signal, int asset_index) {
    ...
    if (total_cost > portfolio->cash) {
        quantity = portfolio->cash / (slippage_price * (1.0 + portfolio->params.tx_cost_pct));
...
static void update_portfolio(WU_Portfolio portfolio, const WU_Signal* signals) {
    WU_BasicPortfolio p = (WU_BasicPortfolio) portfolio;
    int count = p->num_assets;  // Portfolio knows how many assets it manages
    ...
    if (buy_count > 0) {
        bool use_cash_splitting = (p->params.position_sizing.size_type != WU_POSITION_SIZE_PCT_EQUAL &&
                                   p->params.position_sizing.size_type != WU_POSITION_SIZE_STRATEGY_GUIDED);
        double cash_per_signal = use_cash_splitting ? (p->cash / buy_count) : 0.0;
...
double wu_basic_portfolio_asset_value(
    ...
    if (!portfolio || asset_index < 0 || asset_index >= portfolio->num_assets) {
        return 0.0;
    }
